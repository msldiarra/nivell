...
public class TankRepositoryTest {
    ...
    public void add_method_calls_couchbase_client_set_methods_wit_correct_arguments() throws Exception {
        ...
        testRepository.add(expectedTank);

        assertThat(client).setMethodIsCalledWith(expectedTank.getPersistenceKey(), 0, gson.toJson(expectedTank));
    }

    @Test
    public void get_method_calls_couchbase_client_get_methods_wit_correct_arguments() throws Exception {
        ...
        when(connectionManager.getClientInstance()).thenReturn(client);
        when(alertValidator.validate(expectedTank)).thenReturn(new HashSet<ConstraintViolation<Tank>>());
        when(client.get(expectedTank.getIdentifier())).thenReturn(expectedTank);

        TankRepository testRepository = new TankRepository(connectionManager, gson, alertValidator);
        String tank = testRepository.get(expectedTank.getIdentifier());
